...
if (st.session_state.agent_status["data"]
        and not st.session_state.agent_status["analysis"]
        and not st.session_state.no_data_msg
        and company.strip()):
    ...
    st.session_state.no_data_msg = (
        graph_state.get("result", "데이터를 찾을 수 없습니다") if not has_data else ""
    )
    st.rerun()

# 결과 표시
if st.session_state.no_data_msg:
    st.warning(st.session_state.no_data_msg)
